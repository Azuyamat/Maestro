...
private val SUPERSCRIPT_NUMBERS: Map<Char, Char> =
    mapOf(
        '0' to '⁰',
        '1' to '¹',
        '2' to '²',
...
class NumberHelper(private val number: Number) {
    ...
    fun toBar(
        ...
        return "<gray>[$mainColor${separator.repeat(filledBars)}$emptyColor${separator.repeat(emptyBars)}<gray>]"
    }
    ...
    fun toParsedBar(
        ...
        mainColor: String = "<green>",
        emptyColor: String = "<gray>",
    ): Component = toBar(max, bars, separator, mainColor, emptyColor).parse()

    // Convert to superscript
    fun toSuperscript() = number.toString().map { SUPERSCRIPT_NUMBERS[it] }.joinToString("")
}